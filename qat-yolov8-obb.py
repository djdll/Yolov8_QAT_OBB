...
def load_yolov8_model(weight, device) -> OBBModel:
    ...
    model = torch.load(weight, map_location=device)["model"] # 模型必须是训练完整的
...
def build_dataset(cfg, img_path, mode='train', batch=None, gs=32):
    """Build YOLO Dataset."""
    cfg.data = "yourdata.yaml"
    try:
        if cfg.task == 'classify':
            data = check_cls_dataset(cfg.data)
        elif cfg.data.split('.')[-1] in ('yaml', 'yml') or cfg.task in ('detect', 'segment', 'pose','obb'):
...
def export_onnx(model : OBBModel, save_file, size=(640,640), dynamic_batch=False, noanchor=False, prefix=colorstr('ONNX:')):
    ...
    output_names = ['output0','output1','output2'] # or output_names = ['output0']
    dynamic = cfg.dynamic
    if dynamic:
...
def evaluate_coco(model, val_dataloader):
    validator = yolo.obb.OBBValidator(dataloader=val_dataloader, args=cfg)
    val_model = deepcopy(model) # deepcopy
    import ipdb; ipdb.set_trace()
    mAP = validator(model=val_model)["metrics/mAP50-95(B)"]
    return mAP

def predict_coco(model, val_dataloader):
    validator = yolo.obb.OBBPredictor()
    val_model = deepcopy(model)
    validator(source='yourdata_dir',model=val_model)
    return 
...
def run_predict(weight, device, cocodir):
    import ipdb; ipdb.set_trace()
    device  = torch.device(device)
    model   = load_yolov8_model(weight, device)
    val_dataloader = get_dataloader(cfg, cocodir + "images/val", batch_size=cfg.batch, mode='val')
    predict_coco(model, val_dataloader)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cocodir', type=str,  default="/home/dujiang/obb_datasets/v8_refine_20241214/", help="coco directory")
    parser.add_argument("--device", type=str, default="cuda:0", help="device")

    parser.add_argument('--save', type=str,  required=False, help="coco directory") 
    parser.add_argument('--size', type=int,  default=(640,640), help="input size for export onnx ...") # h w
    ...
    parser.add_argument("--nmsthres", type=float, default=0.2, help="nms threshold") 

    parser.add_argument('--export', action='store_true', default=False, help='Export weight to onnx file ...')
    parser.add_argument('--finetune', action='store_true', default=True, help='PTQ/QAT finetune ...')
    parser.add_argument('--sensitive', action='store_true',default=False, help='Sensitive layer analysis ...')
    parser.add_argument('--test', action='store_true', default=False, help='evaluate ...')
    ...
    if args.export:
        ...
        parser.print_help()
# python qat-yolov8-obb.py --weight /home/dujiang/yolo/BarcodeDetectionYolov8/test_iou/finetune_BaseModel_Adam_5e-5_0.05_e350_b256_f0_250116_mc9_box6/weights/best.pt
